...
serve(async (req) => {
    ...
    try {
        ...
        if (event.type === 'checkout.session.completed') {
            ...
            if (mode === 'payment') {
                const creditsToAdd = session.metadata?.credits ? Number(session.metadata.credits) : 0;
                ...
                if (creditsToAdd > 0) {
                    ...
                    console.log(`User ${userId} acquired ${creditsToAdd} VPOs successfully. New balance: ${newCredits}.`);
                } else {
                    console.log(`No credits metadata found for session completed for user ${userId}.`);
                }
            }
        }
